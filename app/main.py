...
from app.capabilities.rerank.registry import RerankRegistry, build_rerank_registry
from app.capabilities.rerank.routes import router as rerank_router
...
async def lifespan(app: FastAPI):
    ...
    app.state.rerank_registry = build_rerank_registry(settings)
    yield
...
def _get_rerank_registry(request: Request) -> RerankRegistry:
...
async def health(
    ...
    rerank_registry = _get_rerank_registry(http_request)

    # All three registries' health checks run concurrently, not
    # sequentially - same reasoning as observability/service.py's
    # build_models_response: this endpoint shouldn't get slower every
    # time a capability gains a new backend.
    generation_backends = generation_registry.all()
    embedding_backends = embedding_registry.all()
    rerank_backends = rerank_registry.all()
    generation_results, embedding_results, rerank_results = await asyncio.gather(
        asyncio.gather(*(b.health_check() for b in generation_backends)),
        asyncio.gather(*(b.health_check() for b in embedding_backends)),
        asyncio.gather(*(b.health_check() for b in rerank_backends)),
    )
    ...
    backends = (
        ...
        + [
            BackendHealth(capability="rerank", backend=r.backend, reachable=r.reachable, detail=r.detail)
